export const countNumberCharacters = n => inWords(n).split('').filter(c => c.match(/\w/)).length;

export const countNumbersCharactersFromRange = (bottom, top) => {
  let counter = 0;
  for (let i = bottom; i <= top; i++) {
    counter += countNumberCharacters(i);
  }
  return counter;
}
